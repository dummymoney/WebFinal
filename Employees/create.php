...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($firstName_err) && empty($lastName_err) && empty($age_err) && empty($gender_err) && empty($phoneNumber_err)&& empty($address_err)&& empty($birthdate_err)&& empty($jobTitle_err)&& empty($salary_err)&& empty($dNum_err)){ 
        ...
        $sql = "INSERT INTO EMPLOYEES (JOBTITLE, FIRST_NAME, LAST_NAME, AGE, SEX, PHONE_NUMBER, EMPLOYEE_ADDRESS, SALARY, BIRTHDATE, dNUM) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($connect, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssisssssi", $param_jobTitle, $param_firstName, $param_lastName, $param_age, $param_gender, $param_phoneNumber, $param_address, $param_salary, $param_birthdate,$param_dNum);
